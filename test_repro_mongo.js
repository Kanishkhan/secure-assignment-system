...
const run = async () => {
    try {
        ...
        const submission = await Submission.create({
            assignment_id: assignment._id, // Pass ObjectId
            student_id: student._id.toString(), // Pass String (simulating req.user.id from JWT)
            filename: 'test.txt',
        ...
        const submissions = await Submission.aggregate([
            ...
            { $unwind: '$student' }, // If this fails, user lookup failed
            {
                $project: {
                    _id: 1,
                    filename: 1,
